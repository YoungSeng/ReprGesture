...
def train_iter_gan(args, epoch, in_text, in_audio, target_poses, vid_indices,
                   ...
                   pose_dec_optim, dis_optim, pitch, energy, volume, speech_emo, text_emo):
    ...
    # print(vid_indices)
    out_dir_vec, z, z_mu, z_logvar, cmd_loss, diff_loss, recon_loss = pose_decoder(pre_seq, in_text, in_audio, vid_indices, pitch, energy, volume, speech_emo, text_emo)
    ...
    if (args.z_type == 'speaker' or args.z_type == 'random') and args.loss_reg_weight > 0.0:
        if args.z_type == 'speaker':
            ...
            rand_idx = torch.randperm(vid_indices.shape[0])     # (128)
            rand_vids = vid_indices[rand_idx]       # (128)
        else:
            rand_vids = None

        out_dir_vec_rand_vid, z_rand_vid, *_ = pose_decoder(pre_seq, in_text, in_audio, rand_vids, pitch, energy, volume, speech_emo, text_emo)
        beta = 0.05
        ...
        if args.use_MISA:
            if args.use_diff:
                loss = 0.9 * (50 * mse_loss + args.loss_regression_weight * huber_loss + 0.01 * torch.mean(diff_loss) + 0.01 * torch.mean(recon_loss) + 0.01 * torch.mean(cmd_loss))  # + var_loss + cmd_loss + diff_loss + recon_loss
            elif args.use_reconstruct:
                loss = 0.9 * (50 * mse_loss + args.loss_regression_weight * huber_loss + 0.01 * torch.mean(recon_loss) + 0.01 * torch.mean(cmd_loss))
            else:
                loss = 0.9 * (50 * mse_loss + args.loss_regression_weight * huber_loss + 0.01 * torch.mean(cmd_loss))
        else:
            loss = 50 * mse_loss + args.loss_regression_weight * huber_loss
